...
const DesignConfigarator = ({imageUrl, configId, imageDimentions} : DesignConfigaratorProps ) => {
    ...
    const [renderedPosistion, setRenderedPosistion] = useState({
        x: 150,
        y: 205,
    })
    ...
    async function saveConfiguration(){
        ...
        try {
            ...
            const actualX = renderedPosistion.x - leftOffset;
            const actualY = renderedPosistion.y - topOffset;
            ...
            ctx?.drawImage(
                userImage, actualX, actualY, renderedDimention.width, renderedDimention.height
            )
  ...
  return (
    <div className='relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20'>
        <div className="relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 
        ...
        >
          <div className="relative w-60 bg-opacity-50 pointer-events-none aspect-[896/1831]">
            <AspectRatio ratio={896 /1831} ref={phoneCaseRef}
            ...
            <ScrollArea className='relative flex-1 overflow-auto'>
                ...
                <div className="px-8 pb-12 pt-8">
                  ...
                  <div className="relative mt-4 h-full flex flex-col justify-between">
                    ...
                    <RadioGroup value={options.color} onChange={(value) => setoptions((prev) => 
                        ({
                            ...prev,
                            color: value
                        }))}>
                            ...
                            <div className="mt-3 flex items-center space-x-3">
                                {COLORS.map((color) => (
                                    <RadioGroup.Option key={color.label} value={color} 
                                        className={({active, checked}) => cn(`relative -m-0.5 flex cursor-pointer items-center justify-center
                                        rounded-full p-0.5 active:ring-0 focus:ring-0 active:outline-none focus:outline-none border-2 border-transparent`, 
                                        {
                                ...
                                <DropdownMenuContent>
                                    {MODELS.options.map((model) => (
                                        <DropdownMenuItem key={model.label} 
                                            className={cn('flex text-sm p-1.5 cursor-default hover:bg-zinc-100',{
                            ...
                            <RadioGroup key={name} value={options[name]} onChange={(value) => {
                                ...
                                <div className="mt-3 space-y-4">
                                    {selectableOptions.map((options) => (
                                        <RadioGroup.Option key={options.value} value={options}
                                        className={({active, checked}) =>
                                         cn(`relative block cursor-pointer rounded-lg bg-white px-6 py-4 border-2 border-zinc-200 focus:outline-none 
                                            ring-0 focus:ring-0 outline-none sm:flex sm:justify-between`,
                                            {
...
export default DesignConfigarator