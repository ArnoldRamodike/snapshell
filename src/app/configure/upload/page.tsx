...
const UploadPage = () => {
    ...
    const onDropRejected = (rejectedFiles: FileRejection[]) =>{
        ...
        toast({
            ...
            description: 'Please use a diffrent file type, PNG/ JPEG',
            variant: 'destructive'
        })
    }
            ...
            >
             {({getRootProps, getInputProps}) => (
                <div className="h-full w-full flex-1 flex-col items-center justify-center" {...getRootProps()}>
                    <input {...getInputProps}/>
                  {isDragOver ? <MousePointerSquareDashedIcon className='h-6 w-6 text-zinc-500 mb-2'/> : 
                  isUploading || isPending ? <Loader2 className='animate-spin h-6 w-6 text-zinc-500 mb-2'/> : <Image className='h-6 w-6 mb-2'/>}
