...
const Review = ({imgSrc, className, ...props}: ReviewProps) => {
    ...
    return(
        <div className={cn('animate-fade-in rounded-[2.25rem] bg-white p-6 opacity-0 shadow-xl shadow-slate-900/5',
             className)} style={{animationDelay}} {...props}>
            <Phone imgSrc={imgSrc}/>
        </div>
    )
}
    ...
    return (
        <div ref={containerRef} className="relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3">
            ...
            <ReviewColumn reviews={[...column1, ...column3.flat(), ...column2]} 
                reviewClassname={(reviewIndex) => cn({"md:hidden": reviewIndex >= column1.length + column3[0].length,
            ...
            <div className="pointer-events-none absolute inset-x-0 to0# h-32 bg-gradient-to-b from-slate-100"/>
            <div className="pointer-events-none absolute inset-x-0 to0# h-32 bg-gradient-to-t from-slate-100"/>
        </div>
    )
}
...
const Reviews = () => {
  return (
    <MaxWidthWrapper className='relative max-w-5xl'>
      <img src="/what-people-are-buying.png" alt="phone cover 1" aria-hidden='true'
